...
resource "azurerm_kubernetes_cluster" "gcc_aks_clusters" {
  ...
  default_node_pool {
    ...
    vnet_subnet_id = var.gcc_subnets[each.value["subnet_key"]].id
  }
...
resource "azurerm_private_dns_zone_virtual_network_link" "gcc_aks_cluster_private_dns_zone_vnet_links" {
  ...
  resource_group_name = "MC_${var.gcc_resource_groups[each.value["rg_key"]].name}_${azurerm_kubernetes_cluster.gcc_aks_clusters[each.value["aks_cluster_key"]].name}_${each.value["aks_cluster_location_name"]}"

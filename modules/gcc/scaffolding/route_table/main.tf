...
resource "azurerm_route_table" "gcc_route_tables" {
  ...
  dynamic "route" {
    ...
    content {
      ...
      next_hop_in_ip_address = route.value["next_hop_resource_type"] == "firewall" ? var.gcc_firewalls[route.value["next_hop_resource_key"]].ip_configuration[0].private_ip_address : var.gcc_linux_vms[route.value["next_hop_resource_key"]].private_ip_address
    }
  }
